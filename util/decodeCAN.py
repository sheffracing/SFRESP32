...
def main():
    try:
        ...
        length_col = find_column(df_bus.columns, ['Length (Bytes)', 'Length (bytes)', 'Length Bytes', 'DLC'])
        ...
        for _, row in df_bus.iterrows():
            ...
            if pid is not None and pd.notna(row['Name']):
                ...
                period_ms = 0
                length_bytes = 8
                if length_col is not None and length_col in row and pd.notna(row[length_col]):
                    try:
                        length_bytes = int(round(float(row[length_col])))
                    except Exception:
                        length_bytes = 8
                if length_bytes <= 0:
                    length_bytes = 8
...
def generate_c_code(messages, msg_map):
    ...
    for pid in sorted(messages.keys()):
        ...
        msg_length_bytes = int(msg_info.get('length_bytes', 8) or 8)
        msg_name_clean = re.sub(r'[^a-zA-Z0-9_]', '', msg_name)
        ...
        c_content += f"    if (stFrame.byDLC != {msg_length_bytes}) return ESP_ERR_INVALID_SIZE;\n"
        ...
        c_content += f"    stFrame.byDLC = {msg_length_bytes};\n"
        c_content += f"    memset(stFrame.abData, 0, {msg_length_bytes});\n\n"
    ...
    for (pid, base_name, rate_hz, period_ms, thresh_ms) in per_msg_list:
        if rate_hz > 10.0:
            bname = f"B{base_name}InError"
            if thresh_ms > 0:
                c_content += f"    tSince{base_name} += 1; if (tSince{base_name} > {thresh_ms}) {bname} = TRUE; else {bname} = FALSE;\n"
            else:
    ...
    for (pid, base_name, rate_hz, period_ms, thresh_ms) in per_msg_list:
        if not (rate_hz > 10.0):
            bname = f"B{base_name}InError"
            if thresh_ms > 0:
                c_content += f"    tSince{base_name} += 100; if (tSince{base_name} > {thresh_ms}) {bname} = TRUE; else {bname} = FALSE;\n"
            else:
